...
JerryScript 在遇到不可恢复的内部错误（如 OOM、断言失败）时会调用 `jerry_port_fatal()` 终止进程。在嵌入式设备上直接终止意味着系统崩溃，因此 ElenixOS 接管了此函数，实现引擎崩溃恢复：**重置引擎到干净状态，使系统继续运行**。

可恢复的致命错误包括：

| 错误码 | JerryScript 定义 | 描述 |
|--------|-----------------|------|
| 10 | `JERRY_FATAL_OUT_OF_MEMORY` | 内存不足 |
| 12 | `JERRY_FATAL_REF_COUNT_LIMIT` | 引用计数上限 |
| 13 | `JERRY_FATAL_DISABLED_BYTE_CODE` | 执行被禁用的指令 |
| 14 | `JERRY_FATAL_UNTERMINATED_GC_LOOPS` | GC 循环上限 |
| 120 | `JERRY_FATAL_FAILED_ASSERTION` | 断言失败 |

#### 恢复原理

恢复机制基于 `setjmp`/`longjmp`：
...
- **`jerry_port_fatal` 位于 `ElenixOS/src/script_engine/jerry_port.c`**：作为强符号覆盖 JerryScript 的弱符号默认实现，执行 `longjmp` 跳回 `script_engine_run()` 的 setjmp 恢复点
- **严格清理顺序**：必须在 `jerry_init()` 重置堆之前释放旧堆上的所有 JS 句柄，否则产生 dangling pointer
- **引擎代数（`engine_gen`）**：每次恢复后自增。SNI 控制块在创建时记录当前代数，`sni_node_from_native()` 和 `sni_obj_deleted_cb()` 通过代数比较识别并拒绝来自旧会话的句柄，防止 use-after-free

#### LVGL 控件清理

引擎崩溃后，SPM 程序被销毁，其关联的 LVGL 控件通过 `cleanup_view` 回调自动清理。回调在程序创建时由上层（如表盘）注册：

```c
typedef struct script_program {
    // ...
    void (*cleanup_view)(void *user_data);   // LVGL 视图清理回调
    void *cleanup_user_data;                 // 回调参数（lv_obj_t *view）
} script_program_t;
```

触发路径：
- `spm_terminate_program()` — 正常终止
- `spm_handle_engine_reset()` — 引擎崩溃时的紧急清理

#### 错误报告

崩溃信息通过以下路径传递给用户：

1. 恢复路径中 `_set_error_info()` 记录包含崩溃代码和描述的字符串（如 `"Engine crash (code=10, Out of memory)"`）
2. `spm_start_program()` 将错误信息保存到持久化副本 `s_last_error` 中
3. 故障面板通过 `spm_get_last_error()` 获取并显示
...
- **空栈帧保护**：若 `jerry_port_fatal` 在 setjmp 帧失效后被调用，直接 `abort()`，避免 longjmp 到无效栈帧
- **递归保护**：若恢复过程中再次发生致命错误（`fatal_recovering == true`），直接 `abort()`，防止 setjmp/longjmp 死循环
...
SPM 将脚本程序状态分为 `ACTIVE`、`SUSPENDED`、`STOPPING` 和 `TERMINATED`。Core 只负责引擎执行状态，不能代替 SPM 判断应用是否仍可恢复；SNI 只负责资源上下文，不能代替 Activity 决定页面导航。

| 状态 | Realm | SNI 资源 | JS 回调 |
| --- | --- | --- | --- |
| `ACTIVE` | 存在 | 可用 | 允许 |
| `SUSPENDED` | 保留 | 保留但暂停 | 禁止 |
| `STOPPING` | 正在销毁 | 按 teardown 清理 | 禁止 |
| `TERMINATED` | 已销毁 | 已释放 | 禁止 |

上层只能通过 `spm_watchface_start/pause/resume/destroy` 和 `spm_app_run/suspend/resume/stop` 等接口改变程序生命周期。直接删除 View、直接调用 JerryScript 或直接销毁 SNI 上下文都不能替代这些路径。

## Recent Apps 与脚本程序

应用切换时，Recent Apps 将 App Activity 子栈停放，SPM 将应用置为 `SUSPENDED`，SNI 保留资源但暂停定时器、动画和回调。恢复时重新附加 Activity 子栈，再按定时器和动画策略恢复资源；恢复不会重新执行应用入口脚本。

应用被卸载、LRU 驱逐或引擎复位时，必须执行完整停止。引擎复位会先保存崩溃上下文，再清理旧程序和所有 Recent Apps 条目；复位之后旧 Realm、JS 值和原生句柄都不可复用。

## SNI 调用约束

SNI 控制块、对象删除通知、JS 引用和异步回调都属于程序上下文。回调执行前必须经过 SPM 状态检查；原生对象删除前必须清除 JS 可见的原生地址；teardown 阶段不能重新进入脚本执行。详细的类型桥接、资源分类、导出描述表和上下文 API 见 [SNI 详细架构](./script_engine/sni)。

## API 文档要求

新增脚本 API 时，文档至少应说明命名空间、参数和返回值、异常/错误行为、返回对象的所有权、可调用的生命周期状态、异步回调的删除规则，以及是否受挂起恢复策略影响。仅记录“某 C 函数被导出”不足以定义脚本 API。
