...
SNI 的资源表同时记录原生地址、JavaScript 对象、资源类型、销毁回调和父子资源关系。资源类型决定它属于对象树、独立托管资源还是混合资源；清理逻辑不能只根据 C 指针是否非空来判断资源是否有效。

### 类型分类

| 类别 | 典型对象 | 所有权与删除 |
| --- | --- | --- |
| 值类型 | 数字、布尔、字符串、颜色、事件值 | 在调用边界复制或转换，不进入原生资源表 |
| 对象树句柄 | `lv_obj_t` 及其子类 | 由 LVGL 对象树删除；SNI 先清除 JS 关联 |
| 受控资源 | 定时器、动画等独立对象 | 由 SNI 上下文登记，删除时调用类型析构逻辑 |
| 混合资源 | 带 JS 回调的 LVGL 资源 | 先中和原生指针，再释放 JS 引用和回调上下文 |

SNI 类型判定接口把句柄、值、对象树节点、纯托管资源和混合资源分开。新增类型时必须同时决定：是否允许作为 JS 参数、是否可以被 GC 触达、原生对象删除时如何通知 SNI、teardown 时由谁最终释放。

### 控制块与失效

每个可被脚本持有的原生对象都有控制块。控制块连接 JS 对象、原生地址、SNI 类型和所属上下文；对象失效时，原生地址必须先清零，之后 JS 仍可能存在的对象只能表现为无效句柄，不能再次访问旧地址。

控制块失效可能由 JS 主动删除、LVGL 父对象级联删除、程序停止、引擎复位或资源驱逐触发。所有路径都必须汇聚到统一注销逻辑，避免重复析构。

## 上下文与类型桥接的接口职责

SNI 上下文接口负责资源登记、按类型遍历、原生地址失效、异步对象删除、挂起/恢复和 teardown。实现代码必须通过上下文提供的生命周期边界操作资源，不能直接改动内部资源链表。

类型桥接接口负责 JS 值与 C 值、值对象和受控句柄之间的转换，并维护控制块、父子资源关系和原生对象删除通知。桥接层不替调用方决定业务所有权；调用方必须明确转换结果的有效期和释放责任。

同步删除只适用于当前调用栈可以安全完成删除的资源；LVGL 回调或遍历过程中不能直接破坏正在使用的对象，应使用异步删除请求，等待安全边界执行。桥接失败时必须停止使用输出对象，不能把类型不匹配的 JS 值强转成原生指针。颜色、事件和 LVGL 结构体等值类型应以当前生成的类型描述为准，不能依赖旧结构体偏移。

## 回调与 GC 安全

SNI 保存的 JS 回调引用必须在所属 Realm 中注册，在资源删除或上下文 teardown 时释放。原生对象删除前要先清除可能被 GC 扫描的指针；GC 之后不得通过失效控制块重新取回原生对象。

定时器和动画回调执行前必须检查上下文、程序状态、控制块状态和原生地址。回调若处于待删除或已删除状态，不得再调用 JS。回调异常由 SPM 记录并决定后续错误路径，SNI 包装器不能吞掉异常后继续使用不确定的原生资源。

## API 导出与生成

SNI API 由描述表导出。描述表包含命名空间、条目名称、参数、返回值、常量和子条目；导出过程包括获取描述表、校验描述、生成或解析 JS 属性、挂载到目标 Realm、为特殊方法绑定包装器，以及在 Realm 销毁时解除挂载。

描述表修改后必须同步类型桥接、生成文件和文档中的参数语义。API 文档应说明参数类型、返回值、错误条件、对象所有权和生命周期，不能只列函数名称。

## SNI 初始化与挂载

`sni_init` 初始化全局类型表、销毁回调和特殊方法；`sni_mount` 把 API 挂载到指定 Realm。全局注册只做一次，Realm 级对象和回调仍必须属于具体的 `sni_context_t`。引擎复位后必须重新建立 Realm 和上下文，不能复用旧控制块。
