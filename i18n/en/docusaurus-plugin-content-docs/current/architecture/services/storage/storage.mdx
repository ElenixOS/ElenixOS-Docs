...
All path resolution is handled at the `eos_fs_port` layer, no longer by the storage service. On the simulator, call `eos_fs_set_root()` once at startup to set the sandbox root; `eos_fs_open_read()` / `eos_fs_open_write()` and friends internally call `eos_fs_realpath()` to transparently map virtual paths to real filesystem paths. On production hardware the root defaults to `"/"`, so paths pass through unchanged.
