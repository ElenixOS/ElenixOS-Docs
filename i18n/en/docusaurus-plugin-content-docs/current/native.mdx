...
安装 `kconfiglib`，用于构建系统自动生成内核配置头文件。未安装时构建不会中断，但回退至内核默认值。

### 基本构建步骤

项目根目录的 `Makefile` 提供快捷命令：
...
底层等价于 `cmake -B build && cmake --build build`。
...
内核编译选项（内存分配策略、文件系统类型、字体源等）通过 Kconfig 系统配置。详见 [内核配置系统](../development/kconfig)。
