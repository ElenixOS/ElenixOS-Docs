...
所有路径在 `eos_fs_port` 层解析，不再由存储服务处理前缀。仿真器中调用 `eos_fs_set_root()` 设置沙箱根目录；`eos_fs_open_read()` / `eos_fs_open_write()` 等函数内部通过 `eos_fs_realpath()` 将虚拟路径透明映射到实际文件系统路径。量产硬件上根目录默认为 `"/"`，路径透传。
